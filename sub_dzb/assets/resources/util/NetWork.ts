...
export class Network {
    ...
    public connect(url: string) {
        ...
        self.socket.onmessage = (evt) => {
            ...
            if (this.USEGZIP && typeof(evt.data) != 'string') {
                if (cc.sys.isNative) {
                    var a = new Uint8Array(evt.data)
                    var inflate = new Zlib.Gunzip(a);
                    var plain = inflate.decompress();
                    let packet: Object = JSON.parse(self.Utf8ArrayToStr(plain))
                    self.appandeMsg(packet);
                    // console.log("packet===========",packet);
                } else {
                    var fileReader = new FileReader();
                    fileReader.onload = function (progressEvent) {
                        var arrayBuffer: ArrayBuffer = this.result;//arrayBuffer即为blob对应的arrayBuffer  
