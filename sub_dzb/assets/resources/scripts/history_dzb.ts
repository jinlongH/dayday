...
export default class history extends cc.Component {
    ...
    initpukejilu(pukejilu: any) {
        ...
        for (var i = 0; i < pukejilu.length;i++) {
            ...
            item.getComponent('historyPage_dzb').initFirstHistory(pukejilu[i].pai1);
            item.getComponent('historyPage_dzb').initTwoHistory(pukejilu[i].pai2);
            item.getComponent('historyPage_dzb').initSelect(pukejilu[i].select);
    ...
    bibeijilu(bibeijilu: any) {
        ...
        for (var i = 0; i < bibeijilu.length; i++) {
            ...
            item.getComponent('historybibeiScrollView_dzb').initBibeiHistory(bibeijilu[i]);
            this.pageview_bibei.addPage(item);
        }
    }
    chujiangjilu(chujiangjilu: any) {
        this.layout_awad.getComponent('historyChujiangjiludzb').setInfo(chujiangjilu);
    }
    ...
    btn_change_machine_Click(event, customEventData) {
        ...
        if (this.layout_change.getComponent("changeMachinedzb"))
        {
            console.log("nick----------------componet")
        }
        this.layout_change.getComponent('changeMachinedzb').gethallInfo();
