...
@ccclass
export default class HistoryPage extends cc.Component {
    ...
    initFirstHistory(history:any){
        for(var i =0;i<history.length;i++){
            this["poker_1_"+(i+1)].getComponent('historycard_dzb').setCard(history[i])
        }
    }
    initTwoHistory(history:any){
        for(var i =0;i<history.length;i++){
            this["poker_2_"+(i+1)].getComponent('historycard_dzb').setCard(history[i])
        }
    }
    initSelect(selsect:any){
        for(var i =0;i<selsect.length;i++){
            this["poker_1_"+selsect[i]].getComponent('historycard_dzb').showHold();
        }
    }
    // update (dt) {}
}
